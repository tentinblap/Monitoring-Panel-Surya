...
@section('title', 'My Panel | Panel 1')
...
//mqtt connecton options including the mqtt broker subscriptions
	var options = {
		...
		onSuccess: function () {
            ...
            client.subscribe(MQTTsubTopic25, {qos: 1});
		},
...
function onMessageArrived(message) {
		...
		if(dataTopics.length<3){
			dataTopics.push("node1/v1"); //add new topic to array
			dataTopics.push("node1/v2"); //add new topic to array
			dataTopics.push("node1/v3"); //add new topic to array
		    
			
			// dataTopics.push(message.destinationName); //add new topic to array
			console.log("qwe"+dataTopics.length)

			
		    var x = dataTopics.indexOf("node1/v1"); //get the index no
		    var y= dataTopics.indexOf("node1/v2"); //get the index no
		    var z= dataTopics.indexOf("node1/v3"); //get the index no
			...
			dataTopics1.push("node1/c1"); //add new topic to array
			dataTopics1.push("node1/c2"); //add new topic to array
			dataTopics1.push("node1/c3"); //add new topic to array
		    
			
			// dataTopics1.push(message.destinationName); //add new topic to array
			console.log("qwe"+dataTopics.length)
			
        var a = dataTopics1.indexOf("node1/c1"); //get the index no
        var b = dataTopics1.indexOf("node1/c2");
        var c = dataTopics1.indexOf("node1/c3");
      ...
      var newseries6 = {
		...
		var z= dataTopics.indexOf("node1/v3"); //get the index no
		var a = dataTopics1.indexOf("node1/c1"); //get the index no
		var b = dataTopics1.indexOf("node1/c2"); //get the index no
		var c = dataTopics1.indexOf("node1/c3"); //get the index no
		// var y = dataTopics.indexOf(message.destinationName); //get the index no of the topic from the array
		var myEpoch = new Date().getTime(); //get current epoch time
			if(message.destinationName=="node1/v1"){
			...
			else if(message.destinationName=="node1/v2"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
			...
			else if(message.destinationName=="node1/v3"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
			...
			else if(message.destinationName=="node1/c1"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
			...
			else if(message.destinationName=="node1/c2"){
						var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
						var plotMqtt = [myEpoch, Number(thenum)]; //create the array
						if (isNumber(thenum)) { //check if it is a real number and not text
						console.log('is a propper number, will send to chart.')
			...
			else if(message.destinationName=="node1/c3"){
						var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
						var plotMqtt = [myEpoch, Number(thenum)]; //create the array
						if (isNumber(thenum)) { //check if it is a real number and not text
						console.log('is a propper number, will send to chart.')
			...
			else if(message.destinationName=="node1/off"){
	...
	function onConnect_ON() {
		...
		var MQTTpubTopic1= "application/1/device/b91ae9bf7ae144d6/tx"; 
		var mqtt_msg ='{     "confirmed": true,     "fPort": 3,     "data": "MQ==" }';
		message = new Paho.MQTT.Message(mqtt_msg);
		message.destinationName = MQTTpubTopic1;
		client.send(message);
		document.getElementById("pubmsg").innerHTML = "topic:" + MQTTpubTopic1  + " " + mqtt_msg + " ...sent";
	}  
...
$(document).ready(function() {
		...
		chart1= new Highcharts.Chart({
	        ...
	        subtitle: {
                                text: 'broker: ' + MQTTbroker + ' | port: ' + MQTTport + ' | topic :  ' + MQTTsubTopic4 + '&'  + MQTTsubTopic5 + '&' + MQTTsubTopic6
                        }.hidden,
	        xAxis: {
	            type: 'datetime',
	            tickPixelInterval: 150,
  ...
  <script src="https://code.jquery.com/jquery-3.5.1.min.js" integrity="sha256-9/aliU8dGd2tb6OSsuzixeV4y/faTqgFtohetphbbj0=" crossorigin="anonymous"></script>
    <script type="text/javascript">
     $(document).ready(function(){
        ...
        $('#OFF').click(function(){
            ...
            .then((willDelete) => {
              if (willDelete) {
                $.ajax({ url: ' /api/v1/devices/node', method: 'PUT', data: 'status=0'})
    ...
    $(document).ready(function(){
      ...
      $('#OFF1').click(function(){
          ...
          $.ajax({ url: ' /api/v1/devices/lampu', method: 'PUT', data: 'status=0'})
            .then(function(response) {
               console.log(response);
            });
      });
      $('#ON1').click(function(){
          ...
          $('#ON1').attr('disabled','disabled');
          $.ajax({ url: ' /api/v1/devices/lampu', method: 'PUT', data: 'status=1'})
            .then(function(response) {
              console.log(response);
