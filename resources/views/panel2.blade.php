...
//mqtt connecton options including the mqtt broker subscriptions
	var options = {
		...
		onSuccess: function () {
            ...
            client.subscribe(MQTTsubTopic26, {qos: 1});
		},
...
function onMessageArrived(message) {
		...
		if(dataTopics.length<3){
			dataTopics.push("node2/v1"); //add new topic to array
			dataTopics.push("node2/v2"); //add new topic to array
			dataTopics.push("node2/v3"); //add new topic to array
		    ...
		    var x = dataTopics.indexOf("node2/v1"); //get the index no
		    var y= dataTopics.indexOf("node2/v2"); //get the index no
		    var z= dataTopics.indexOf("node2/v3"); //get the index no
			...
			dataTopics1.push("node2/c1"); //add new topic to array
			dataTopics1.push("node2/c2"); //add new topic to array
			dataTopics1.push("node2/c3"); //add new topic to array
		    
			
			// dataTopics1.push(message.destinationName); //add new topic to array
			console.log(dataTopics.length)
			
        var a = dataTopics1.indexOf("node2/c1"); //get the index no
        var b = dataTopics1.indexOf("node2/c2");
        var c = dataTopics1.indexOf("node2/c3");
      ...
      var newseries6 = {
		...
		var z= dataTopics.indexOf("node2/v3"); //get the index no
		var a = dataTopics1.indexOf("node2/c1"); //get the index no
		var b = dataTopics1.indexOf("node2/c2"); //get the index no
		...
		if(nodeoff){
			...
			else if(message.destinationName=="node2/v2"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
			...
			else if(message.destinationName=="node2/v3"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
			...
			else if(message.destinationName=="node2/c1"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
      ...
      else if(message.destinationName=="node2/c2"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
      ...
      else if(message.destinationName=="node2/c3"){
				var thenum = message.payloadString.replace( /^\D+/g, ''); //remove any text spaces from the message
				var plotMqtt = [myEpoch, Number(thenum)]; //create the array
				if (isNumber(thenum)) { //check if it is a real number and not text
				console.log('is a propper number, will send to chart.')
		...
		else if(message.destinationName=="node2/off"){
...
$(document).ready(function() {
	    chart = new Highcharts.Chart({
	        ...
	        subtitle: {
                                text: 'broker: ' + MQTTbroker + ' | port: ' + MQTTport + ' | topic : ' + MQTTsubTopic7 + '&'  + MQTTsubTopic8
                        }.hidden,
	        xAxis: {
	            type: 'datetime',
		...
		chart1= new Highcharts.Chart({
	        ...
	        subtitle: {
                                text: 'broker: ' + MQTTbroker + ' | port: ' + MQTTport + ' | topic : ' + MQTTsubTopic9 + '&'  + MQTTsubTopic10
                        }.hidden,
	        xAxis: {
	            type: 'datetime',
	            tickPixelInterval: 150,
